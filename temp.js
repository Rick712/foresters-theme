if (!('jQuery' in window)) throw 'Dexels feed library requires jQuery 1.8.0';
...
(function ($, undefined) {
    ...
    var parser = {
        ...
        parse: function (string, separator, assignment) {
            if ($.type(string) !== 'string' || string.length === 0) return {};
            ...
            for (var i = 0; i < components.length; i++) {
                ...
                if (value === 'true') result[decodedKey] = true;
                else if (value === 'false') result[decodedKey] = false;
                else if (value != null)
                    result[decodedKey] = decodeURIComponent(value);
                else result[decodedKey] = null;
            }
    ...
    var token = (function () {
        ...
        if (parsed != null) {
            ...
            if (differenceInMS <= 1000 * 60) return persistence.removeToken();
    ...
    var template = {
        execute: function ($element, article, list) {
            ...
            $.each(parameters, function (key) {
                ...
                if (remove) delete parameters[key];
            });

            if ($.isArray(article.scopes))
                $.extend(parameters, { token: token });
            else $.extend(parameters, { client_id: settings.clientId });

            server
                .get(article.name, parameters)
                .done(function (data, jqXHR) {
                    broadcast.notifyResult(
                        article,
                        data,
                        $element,
                        article,
    ...
    var URI = (function () {
        ...
        return function (mode) {
            if (!(mode in backend)) throw 'Unknown mode ' + mode;
    ...
    feed = (function () {
        return {
            ...
            logout: function () {
                ...
                return $.getJSON(
                    URI(settings.mode).auth + '/oauth/logout?token=' + token
                ...
                $.each(list, function () {
                    if (typeof this.scopes !== 'undefined')
                        $.each(this.scopes, function () {
                            if (scopes.indexOf(this) == -1) scopes.push(this);
                        });
                });
                scopes = scopes.join(',');

                return (
                    URI(settings.mode).auth +
                    '/oauth?redirect_uri=' +
                    redirect_uri +
                    '&response_type=token' +
                    '&client_id=' +
...
(function ($, undefined) {
    ...
    (function () {
        ...
        !(function (a, b) {
            ...
            function r(a) {
                return (
                    a instanceof q || (null != a && null != a._isAMomentObject)
                );
            }
            function s(a) {
            ...
            function jb(b) {
                ...
                var c,
                    ...
                    j = 0;
                for (
                    e = X(b._f, b._locale).match(Bd) || [], c = 0;
                    c < e.length;
                    c++
                )
                    (f = e[c]),
            ...
            function lb(a) {
                ...
                if (0 === a._f.length)
                    return (
                        (l(a).invalidFormat = !0), void (a._d = new Date(NaN))
                    );
            ...
            function zb(b, c) {
                ...
                return c._isUTC
                    ? ((d = c.clone()),
                      (e =
                          (r(b) || f(b) ? b.valueOf() : rb(b).valueOf()) -
                          d.valueOf()),
            ...
            function Ib() {
                ...
                if ((p(a, this), (a = ob(a)), a._a)) {
                    ...
                    this._isDSTShifted =
                        this.isValid() && u(a._a, b.toArray()) > 0;
            ...
            function Ub(b, c) {
                ...
                return this.format(
                    g || this.localeData().calendar(f, this, rb(d))
                );
            }
            function Vb() {
                return new q(this);
            ...
            function hd(a, b, c) {
                var d = Mb(a).abs(),
                    ...
                    k = (e < kf.s && ['s', e]) ||
                        ...
                        (1 >= j && ['y']) || ['yy', j];
                return (
                    (k[2] = b), (k[3] = +a > 0), (k[4] = c), gd.apply(null, k)
                );
            }
            ...
            var fe = /D[oD]?(\[[^\[\]]*\]|\s+)+MMMM?/,
                ...
                he = 'Jan_Feb_Mar_Apr_May_Jun_Jul_Aug_Sep_Oct_Nov_Dec'.split(
                    '_'
                ),
                ie = Vd,
                je = Vd;
        ...
        getObjectDatasource: function (
            ...
            return {
                ...
                getHeaderAtIndexPath: function (column) {
                    ...
                    if (column === 0) return ui.label($element, defaults);
                    return undefined;
                },
    ...
    Table.prototype.header = function () {
        ...
        for (var i = 0; i < this.datasource.columnCount; i++)
            $tr.append(
                $('<th>').append(this.datasource.getHeaderAtIndexPath(i))
            );
    ...
    Table.prototype.body = function () {
        if (!$.isFunction(this.datasource.getCellAtIndexPath)) return undefined;
        ...
        for (var row = 0; row < this.datasource.rowCount; row++) {
            ...
            if (isOwn) $tr.addClass('Own');

            for (
                var column = 0;
                column < this.datasource.columnCount;
                column++
            ) {
    ...
    Table.prototype.toHTML = function () {
        if (typeof this.datasource === 'undefined') return undefined;
    ...
    var ui = {
        ...
        outputLink: function ($element, defaults, value) {
            ...
            if (link === undefined) link = value;
            else if (link.indexOf('?') == -1)
                link += '?' + value.replace(/.*\?/, '');
            else link += '&' + value.replace(/.*\?/, '');
    ...
    this.broadcast.listen(
        'error',
        function (data, jqXHR, $element, article, list) {
            if (data != 'null') {
    ...
    this.broadcast.listen(
        'result',
        function (data, $element, article, list, jqXHR) {
            this.log('Result %s', article.name);

            var contentType = jqXHR.getResponseHeader('content-type');

            if ($.isArray(data)) {
                var source = datasources.getTableDatasource(
                    $element,
                    article,
                    data
                );
                ...
                var sources = datasources.getObjectDatasources(
                    $element,
                    article,
                    data,
                    list
